...
struct CmagVersion {
    ...
    static std::optional<CmagVersion> fromString(const std::string &arg) {
        ...
        int numberOfCharacterRead{};
        const int result = sscanf(arg.c_str(), "%u.%u.%u%n", &compValue0, &compValue1, &compValue2, &numberOfCharacterRead);
        if (result != 3) {
            return {};
        }
        if (static_cast<size_t>(numberOfCharacterRead) != arg.size()) {
            return {};
        }
    ...
    bool isProjectCompatible(const CmagVersion other) const {
        return (comp0 == other.comp0) && (comp1 == other.comp1);
    }
